...
const Button = ({ name, variant, className, icon, type = 'button', handleClick }: ButtonProps) => {
    ...
    const genClassVariant = (variant: string | undefined) => {
        switch (variant) {
            ...
            case 'default':
                return 'bg-gradient-fire text-white '
            case 'disabled':
    ...
    return (
        <button
            className={`h-fit overflow-hidden px-3 py-2 text-sm relative transition-all rounded-[10px] active:scale-95 ${genClassVariant(
                variant
            )} ${className}`}
            onClick={(e) => {
